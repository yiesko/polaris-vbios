...
mod tests {
    ...
    /// Builds a minimal ROM image: MCT at `mct`, one command table at
    /// `table` with the given code bytes.
    fn rom_with_table(mct: usize, table: usize, code: &[u8]) -> Vec<u8> {
        let mut buf = vec![0u8; table + 6 + code.len() + 0x100];
        buf[0] = 0x55;
        buf[1] = 0xAA;
        let size = 6 + code.len();
        assert!(size <= u16::MAX as usize);
        buf[table] = (size & 0xFF) as u8;
        buf[table + 1] = (size >> 8) as u8;
        buf[table + 2] = 1; // fmt rev
        buf[table + 3] = 1; // cont rev
        buf[table + 4] = 4; // ws
        buf[table + 5] = 0; // ps
        buf[table + 6..table + 6 + code.len()].copy_from_slice(code);
        buf[mct] = (4 + 81 * 2) as u8; // header size
        buf[mct + 2] = 1; // fmt rev
        buf[mct + 4] = (table & 0xFF) as u8;
        buf[mct + 5] = (table >> 8) as u8;
        buf
    }

    /// The zero-operand opcodes (NOP, REPEAT, BEEP, SAVE_REG,
    /// RESTORE_REG, DEBUG) and the 0x67-0x78 families must decode
    /// without hanging or desyncing (regression for the SHL/SHR
    /// operand bug).
    #[test]
    fn decodes_zero_op_and_shift_families() {
        let code = vec![
            0x5A, // NOP
            0x53, // REPEAT (unimplemented)
            0x62, 0x11, // POST_CARD 0x11
            0x63, // BEEP
            0x64, // SAVE_REG
            0x65, // RESTORE_REG
            0x66, 0x0A, // SET_DATA_BLOCK 10
            0x67, 0x00, 0x01, 0x00, 0x02, 0x00, // XOR_REG REG[0x0001], REG[0x0002]
            0x6D, 0x00, 0x03, 0x00, 0x04, 0x00, // SHL_REG REG[0x0003], REG[0x0004]
            0x73, 0x00, 0x05, 0x00, 0x06, 0x00, // SHR_REG REG[0x0005], REG[0x0006]
            0x79, // DEBUG
            0x5B, // EOT
        ];
        let buf = rom_with_table(0x200, 0x300, &code);
        let r = Reader::new(&buf);
        let tables = disasm_command_tables(&r, 0x200, None, None).expect("decodes cleanly");
        ...
        // EOT (0x5B) ends the stream without a line of its own.
        assert!(
            tables[0]
                .lines
                .last()
                .is_some_and(|l| l.text.starts_with("DEBUG")),
            "last line should be DEBUG, got: {:?}",
            tables[0].lines.last()
        );
        // The SHL/SHR operand is a full src argument, not a raw shift
        // byte (kernel atom_op_shl/atom_op_shr semantics).
        assert!(joined.contains("REG[0x0003], REG[0x0004]"), "{joined}");
        assert!(joined.contains("REG[0x0005], REG[0x0006]"), "{joined}");
    }

    /// A SWITCH whose case list never reaches the 0x5A5A terminator
    /// must bail out at the table end instead of reading past it.
    #[test]
    fn unterminated_switch_bails() {
        let code = vec![
            0x42, 0x00, 0x00, 0x00, // SWITCH attr 0, src REG 0x0000
            0x63, 0x01, 0x00, 0x00, 0x00, 0x10, 0x00, // case 1 -> +0x10
            0x63, 0x02, 0x00, 0x00, 0x00, 0x20, 0x00, // case 2 -> +0x20 (no terminator)
        ];
        let buf = rom_with_table(0x200, 0x300, &code);
        let r = Reader::new(&buf);
        let err = disasm_command_tables(&r, 0x200, None, None).expect_err("unterminated SWITCH");
        assert!(
            format!("{err:#}").contains("unterminated SWITCH"),
            "{err:#}"
        );
    }

    /// A table that is nothing but zero-length opcodes must terminate
    /// with a clean error, never hang: the loop must advance one byte
    /// per instruction and stop at the table end.
    #[test]
    fn zero_length_storm_is_bounded() {
        let code = vec![0x5A; 2048];
        let buf = rom_with_table(0x200, 0x300, &code);
        let r = Reader::new(&buf);
        let err = disasm_command_tables(&r, 0x200, None, None).expect_err("bounded");
        assert!(
            format!("{err:#}").contains("ran off the end"),
